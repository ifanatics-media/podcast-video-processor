...
export default async function handler(request, response) {
    ...
    try {
        ...
        if (!audioRes.ok || !artworkRes.ok) throw new Error(`Failed to download assets. Audio: ${audioRes.statusText}, Artwork: ${artworkRes.statusText}`);
        ...
        Promise.all([fs.unlink(audioPath), fs.unlink(artworkPath), fs.unlink(subsPath), fs.unlink(outputPath)]).catch(err => console.error("Error cleaning up temp files:", err));
    }
}